...
JPA는 어플리케이션과 데이터베이스 사이에서 동작한다. 따라서 사이 계층으로 동작하면서 최적화할 수 있는 부분이 많다.
...
그러나 JPA는 어플리케이션과 데이터베이스 간 추상화된 데이터 접근 계층을 제공해 특정 데이터베이스 기술에 종속되지 않도록 한다. JPA가 어차피 SQL과 관련된 모든 일을 하니 어플리케이션에 미치는 영향이 없다.
